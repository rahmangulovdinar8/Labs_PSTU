...
Money Money::operator+(const Money& m) const {
    long total_kop = rubles * 100 + kopecks + m.rubles * 100 + m.kopecks;
    return Money(total_kop / 100, total_kop % 100);
}

Money Money::operator/(int n) const {
    if (n == 0) return Money(0,0);
    long total_kop = rubles * 100 + kopecks;
    total_kop /= n;
    return Money(total_kop / 100, total_kop % 100);
}

bool Money::operator>(const Money& m) const {
    if (rubles != m.rubles) return rubles > m.rubles;
